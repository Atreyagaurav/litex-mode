...
- =(units-convert-simple (/ 1 2) "m" "ft") ⇒ \frac{1}{2} \text{m -> ft}=
- =(units-convert "2 m" "ft") ⇒ 2 m \text{(-> ft)}=
- =(units-reduce "1 m + 24 in") ⇒ 1 m + 24 in=
- =(units-ignore 5 "ft") ⇒ 5 \text{ft}=
...
If you're using slime for evaluation, you need to load the units functions in slime too. The functions are available in units-mode's github.
